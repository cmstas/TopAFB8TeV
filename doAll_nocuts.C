void doAll_nocuts(TString outputDir="results", bool rundata=false, bool runsig=false, bool runmc = false, bool requireBTag=true, bool require2BTag=false, bool usePF = true, 
...
if(runttdil) {
    ...
    if(runskim){
	  ...
	  cout << "Doing the Summer12 Madgraph ttbar skim" << endl;
	  ...
	  cout << "Doing the Summer12 Madgraph ttbar full sample" << endl;
	  ch_ttbar->Add("/hadoop/cms/store/group/snt/papers2012/Summer12_53X_MC/TTJets_FullLeptMGDecays_8TeV-madgraph_Summer12_DR53X-PU_S10_START53_V7A-v2/V05-03-24/merged*root");
    ...
    //for mc@NLO
    baby->ScanChain(ch_ttbar, v_Cuts, "ttdil",doFRestimation, lumiToNormalizeTo*(154./157.5)*(190.41256/147.4), kttdil, false);
	//must mutliply by ratio of per-event xsec and PREP xsec to account for negative weights
