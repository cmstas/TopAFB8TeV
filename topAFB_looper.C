...
double topAFB_looper::TopPtWeight(double topPt)
{
  ...
  double result = exp(0.156 - 0.00137 * topPt); //8 TeV fit
  return result;
}

pair<float,float> topAFB_looper::getPhiCorrMET( float met, float metphi, int nvtx, bool ismc ) {                                                 
...
void topAFB_looper::ScanChain(TChain* chain, vector<TString> v_Cuts, string prefix, 
 ...
 if (createBabyNtuples && !applyNoCuts){
  ...
  if(prefix=="data"){
    ...
    set_goodrun_file_json("Cert_190456-208686_8TeV_PromptReco_Collisions12_JSON.txt"); //taken from TPrime
    cout << "DATA!!!" << endl;
     isData = true;    
  }

  /*
  if(prefix == "ttdil" || prefix == "ttotr") {     //commented out to be like tprime
  ...
  */
  ...
  set_vtxreweight_rootfile("vtxreweight_Summer12MC_PUS10_19fb_Zselection.root", false, false); //taken from tprime
  //set_vtxreweight_rootfile("vtxreweight_Summer11MC_PUS4_4p7fb_Zselection.root",false);
  //set_vtxreweight_rootfile("vtxreweight_Fall11MC_PUS6_4p7fb_Zselection.root",false);
  ...
  //------------------------------------------------------------------------------------------------------
  // latest-and-greatest JEC, taken from tprime
  //------------------------------------------------------------------------------------------------------
  ...
  while(TChainElement *currentFile = (TChainElement*)fileIter.Next() ) {
	...
	print_evt_weight = false;
      ...
      float ndavtxweight = vtxweight(isData);
	...
	if (isatty(1)) {
     ...
     if(!applyNoCuts){
      ...
      //if( !cleaning_standardApril2011() )                            continue;   //This causes problems. Instead, use the below, which skips the problematic track selections.
                                                                              	   //Track selectiosn are probably redundant anyway, according to Yanjun
	...
	if( !passSUSYTrigger2012_v1( hyp_type()[ihyp] ) ) continue;  //This is the up-to-date trigger, from tprime
	...
	if(usepfJets) {
	  ...
	  */
	  ...
	  for (unsigned int i = 0; i < pfjets_p4().size(); i++) {
		...
		jet_corrector_pfL1FastJetL2L3->setJetPt ( cms2.pfjets_p4().at(i).pt()  );
		jet_corrector_pfL1FastJetL2L3->setJetEta( cms2.pfjets_p4().at(i).eta() );
		double corr = jet_corrector_pfL1FastJetL2L3->getCorrection();
		pfjet_corr.push_back(corr);
		v_jetP4s.push_back(pfjets_p4()[i] * corr *globalJESRescale);
	  }
...
*/	
	...
	pair<float, float> p_met, p_met_phicorr; //met and met phi
	if(usetcMET || usepfMET || usetcMET35X) {
	  //p_met = getMet(metAlgo, hypIdx);
	  p_met = met_corrector_pfL1FastJetL2L3->getCorrectedMET();
	  p_met_phicorr = getPhiCorrMET( p_met.first, p_met.second, ndavtx, !isData);
	...
	for(unsigned int i = 0; i < v_goodJets.size(); i++) {
	  v_goodJets_p4.push_back(pfjets_p4()[v_goodJets[i]]*pfjet_corr[v_goodJets[i]]*globalJESRescale);
	}
	...
	for(unsigned int i = 0; i < v_goodBtagJets.size(); i++) {
	  v_goodBtagJets_p4.push_back(pfjets_p4()[v_goodBtagJets[i]]*pfjet_corr[v_goodBtagJets[i]]*globalJESRescale);
	}
	...
	for(unsigned int i = 0; i < v_goodNonBtagJets.size(); i++) {
	  v_goodNonBtagJets_p4.push_back(pfjets_p4()[v_goodNonBtagJets[i]]*pfjet_corr[v_goodNonBtagJets[i]]*globalJESRescale);
	}
	...
	for(unsigned int i = 0; i < v_goodJetsNoEtaCut.size(); i++) {
	  v_goodJetsNoEtaCut_p4.push_back(pfjets_p4()[v_goodJetsNoEtaCut[i]]*pfjet_corr[v_goodJetsNoEtaCut[i]]*globalJESRescale);
	}
